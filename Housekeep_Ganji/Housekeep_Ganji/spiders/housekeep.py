...
class HousekeepSpider(scrapy.Spider):
    name = 'housekeep'
    ...
    start_urls = [
        'http://{city}.ganji.com/banjia/'.format(city=city) for city in all_city
    ]
    ...
    def parse(self, response):
        link = LinkExtractor(allow='/banjia/.*',restrict_xpaths='//dl[@class="selitem selitem-area clearfix"]/dd[@class="posrelative w-area"]/a')
    ...
    def get_index(self, response):
        ...
        link = LinkExtractor(restrict_xpaths='//div[@class="leftBox"]//div[@class="list"]/ul/li[@class="list-img"]/div[@class="pic"]/a')
        links = link.extract_links(response)
    ...
    def get_message(self, response):
        ...
        lable = response.xpath('//div[@class="d-top-info"]/ul//span[contains(text(),"服务特色")]/following-sibling::div//text()').extract_first()
        if lable:
            item['label'] = lable
        else:
            item['label'] = ''
        service = response.xpath('//div[@class="d-top-info"]/ul//span[contains(text(),"提供服务")]/following-sibling::div/a/text()').extract()
        if service:
            item["service"] = "".join(service)
        else:
            item['service'] = ''
        item['address'] = "".join(response.css('.d-top-info > ul > li:nth-last-child(2) > div > p ::text').extract())
        s_phone = response.xpath('//div[@class="d-top-info"]//li[@class="tel-num clearfix"]/div[@class="tcon pos-r"]/a/@gjalog').extract_first().strip()
